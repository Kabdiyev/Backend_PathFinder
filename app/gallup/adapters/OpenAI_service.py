...
class OpenAIService:
    def professions_list(self, galluptext, top3prof, MBTI_str, MIT):
        ...
        text2 = re.sub(r'^.*?\n', '\n', conversation.predict(input=f"continue to say about which top 3 potential career field paths may be considered, minimum length of report should be 400 words, at the beggining it should have small introduction"))
        text3 = re.sub(r'^.*?\n', '\n', conversation.predict(input=f"Based on my gallup strengths and Multiple Intelligences Test results analyse my top 5 selected professions {top3prof}, minimum length of report should be 400 words, at the beggining it should have small introduction"))
